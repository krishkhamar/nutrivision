...
const DashboardScreen = ({ onNavigate, screenData }) => {
  ...
  const [bmr, setBmr] = useState(biometrics?.bmr || user?.bmr || 1650);
  const [tdee, setTdee] = useState(biometrics?.tdee || user?.tdee || 2300);
  const [bmi, setBmi] = useState(biometrics?.bmi || user?.bmi);
  const [goalPlan, setGoalPlan] = useState(user?.goalPlan || biometrics?.goalPlan || null);
  const [activeTab, setActiveTab] = useState('Home');
  const calorieRemaining = Math.max(0, dailyCalorieTarget - loggedCalories);
  const calorieOverage = Math.max(0, loggedCalories - dailyCalorieTarget);
  const calorieProgress = dailyCalorieTarget > 0 ? Math.min(100, Math.round((loggedCalories / dailyCalorieTarget) * 100)) : 0;
    ...
    try {
      ...
      if (data) {
        if (data.user) {
          ...
          if (data.user.bmr) setBmr(data.user.bmr);
          if (data.user.tdee) setTdee(data.user.tdee);
          if (data.user.bmi) setBmi(data.user.bmi);
          if (data.user.goalPlan) setGoalPlan(data.user.goalPlan);
        }
        ...
        if (data.nutrition && typeof data.nutrition.caloriesConsumed === 'number') setLoggedCalories(data.nutrition.caloriesConsumed);
      }
    } catch (error) {
      ...
      try {
        ...
        if (savedWater !== null) setWaterIntake(parseFloat(savedWater) || 0);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.headerBanner}>
          <View style={styles.bannerTextContainer}>
            <Text style={styles.bannerTitle}>{greeting}, {firstName}! 👋</Text>
        ...
        {/* Calorie budget: sourced from today's saved meals and the user's biometrics target. */}
        <View style={styles.calorieBudgetCard}>
          <View style={styles.calorieBudgetHeader}>
            <View>
              <Text style={styles.calorieBudgetLabel}>TODAY'S CALORIE BUDGET</Text>
              <Text style={styles.calorieBudgetValue}>{loggedCalories} <Text style={styles.calorieBudgetUnit}>/ {dailyCalorieTarget} kcal eaten</Text></Text>
            </View>
            <View style={[styles.remainingBadge, calorieOverage > 0 && styles.overBudgetBadge]}>
              <Text style={[styles.remainingText, calorieOverage > 0 && styles.overBudgetText]}>{calorieOverage > 0 ? `${calorieOverage} over` : `${calorieRemaining} left`}</Text>
            </View>
          </View>
          <View style={styles.calorieTrack}>
            <View style={[styles.calorieFill, { width: `${calorieProgress}%` }, calorieOverage > 0 && styles.overBudgetFill]} />
          </View>
          <Text style={styles.calorieBudgetHint}>{calorieOverage > 0 ? 'You have exceeded today\'s calorie target.' : 'Log meals in Nutrition to keep this up to date.'}</Text>
          <Text style={styles.goalSummary}>{goalPlan?.direction === 'gain' ? `Gain plan: ${Math.abs(goalPlan.dailyAdjustment)} kcal/day surplus` : goalPlan?.direction === 'loss' ? `Loss plan: ${Math.abs(goalPlan.dailyAdjustment)} kcal/day deficit` : 'Maintenance plan'} · TDEE {tdee} kcal{bmi ? ` · BMI ${bmi}` : ''}</Text>
        </View>
        ...
        {/* 3. Summary Cards */}
        {/* Workout plan is informational until workout logging is added. */}
        <View style={styles.summaryCard}>
          ...
          <TouchableOpacity style={styles.cardActionButton} activeOpacity={0.8} onPress={() => onNavigate('workout')}>
        ...
        <View style={styles.summaryCard}>
          ...
          <TouchableOpacity style={styles.cardActionButtonAlt} activeOpacity={0.8} onPress={() => onNavigate('nutrition')}>
...
const styles = StyleSheet.create({
  ...
  overBudgetBadge: { backgroundColor: 'rgba(239, 68, 68, 0.16)' },
  remainingText: { color: '#10b981', fontSize: 13, fontWeight: '800' },
  overBudgetText: { color: '#f87171' },
  calorieTrack: {
    height: 9,
    backgroundColor: '#0f172a',
    borderRadius: 5,
  ...
  calorieFill: { height: '100%', backgroundColor: '#10b981', borderRadius: 5 },
  overBudgetFill: { backgroundColor: '#ef4444' },
  calorieBudgetHint: { color: '#64748b', fontSize: 12, marginTop: 9 },
  goalSummary: { color: '#38bdf8', fontSize: 12, fontWeight: '700', marginTop: 7 },
  progressRingRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 24,
