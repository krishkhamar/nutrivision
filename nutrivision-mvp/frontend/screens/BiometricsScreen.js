...
const BiometricsScreen = ({ onNavigate, screenData }) => {
  const existingUser = screenData?.user || {};
  const [currentStep, setCurrentStep] = useState(1);

  // Step 1: Personal Info
  const [gender, setGender] = useState(existingUser.gender || 'male');
  const [dob, setDob] = useState(existingUser.dob ? new Date(existingUser.dob) : new Date(2003, 0, 1));
  const [showDatePicker, setShowDatePicker] = useState(false);

  // Step 2: Body Info
  const [height, setHeight] = useState(existingUser.height ? String(existingUser.height) : '');
  const [currentWeight, setCurrentWeight] = useState(existingUser.currentWeight ? String(existingUser.currentWeight) : '');
  const [targetWeight, setTargetWeight] = useState(existingUser.targetWeight ? String(existingUser.targetWeight) : '');

  // Step 3: Pathway & Strategy
  const [pathway, setPathway] = useState(existingUser.pathway || 'Diet Deficit Only');

  // Step 4: Activity Level
  const [activityLevel, setActivityLevel] = useState(existingUser.activityLevel || 1.2);

  // Step 5: Health & Fitness Questions
  const [dietaryPreference, setDietaryPreference] = useState(existingUser.dietaryPreference || 'Non-Veg');
  const [workoutFrequency, setWorkoutFrequency] = useState(existingUser.workoutFrequency || '3-4 days/week');
  const [medicalConditions, setMedicalConditions] = useState(existingUser.medicalConditions || '');
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        onNavigate('goal-plan', { user, biometrics });
      } else {
