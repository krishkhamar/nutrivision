...
exports.saveBiometrics = async (req, res) => {
  try {
    ...
    const { dob, age, height, currentWeight, targetWeight, pathway, gender, activityLevel, dietaryPreference, workoutFrequency, medicalConditions } = req.body;
    ...
    // Calculate daily calorie target based on pathway
    let calorieAdjustment = 0;
    const pathwayLower = String(p).toLowerCase();
    if (pathwayLower.includes('extreme') && pathwayLower.includes('loss')) {
      calorieAdjustment = -750;
    } else if (pathwayLower.includes('loss') || pathwayLower.includes('lose') || pathwayLower.includes('deficit')) {
      calorieAdjustment = -500;
    } else if (pathwayLower.includes('hybrid') || pathwayLower.includes('recomp') || pathwayLower.includes('balance')) {
      calorieAdjustment = -250;
    } else if (pathwayLower.includes('gain')) {
      calorieAdjustment = 500;
    }
